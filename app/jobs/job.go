...
import (
	...
	"maidianba-fluctuate/app/logic"
	"maidianba-fluctuate/app/mail"
...
func NewCommandJob(id int, name string, command string) *Job {
	...
	if strings.HasPrefix(command, local) {
		job.runFunc = func(timeout time.Duration) (string, string, error, bool) {
			beego.Info("local方法执行！")

			funcName := strings.Replace(command, local, "", 1)
			err := logic.HandlerMap[funcName].Handle()
			resultF := "ok，执行command：" + command
			resultS := "ok"
			//if len(result) >0{
			//	resultF = result[0].String()
			//}
			//if len(result) >1{
			//	resultS = result[1].String()
			//}
			return resultF, resultS, err, false
		}
	} else {
		job.runFunc = func(timeout time.Duration) (string, string, error, bool) {
